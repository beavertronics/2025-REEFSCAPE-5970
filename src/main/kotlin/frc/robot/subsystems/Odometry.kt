...
object `according to all known laws of aviation, our robot should not be able to fly` : SubsystemBase() {
    ...
    val leftVel get() =  Drivetrain.leftEncoder.rate.metersPerSecond
    val rightVel get() = Drivetrain.rightEncoder.rate.metersPerSecond
    ...
    fun reset(x: DistanceUnit, y: DistanceUnit, theta: AngleUnit) {
        ...
        poseProvider.resetPosition(navx.rotation2d, Drivetrain.leftEncoder.distance, Drivetrain.rightEncoder.distance, p)
    }
    ...
    override fun periodic() {
        pose = poseProvider.update(navx.rotation2d, Drivetrain.leftEncoder.distance, Drivetrain.rightEncoder.distance)
    }
